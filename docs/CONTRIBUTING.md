...
We welcome all contributions, including pull requests, bug reports, and discussions. Thank you for helping improve **IRENE-Sankey**!
...
✔ **Prefix relevant component** → (e.g., `[docs]`, `[core]`, `[plots]`).  

Example:  
```bash
[core] Improve node alignment and flow accuracy in Sankey diagrams  

Adjusted node positioning to prevent overlap and improve readability  
Refined flow calculations to ensure correct proportional representation  
Fixed issue with inconsistent node ordering in complex graphs  
Resolves: #123
```
...
🙌 Thank you for contributing to **IRENE-Sankey**!